...
DBHandler::~DBHandler() { sqlite3_close(db_); }
...
const std::string DBHandler::Execute_(const std::string& query,
...
const std::string DBHandler::UpdateTask(std::vector<std::string> values) {
  if (IsTaskExist_(values.at(0))) {
    std::string query = 
      "update tasks set name='" + values.at(1) +
      "', description='" + values.at(2) + "', date='" +
      values.at(3) + "', category='" + values.at(4) +
      "' where name='" + values.at(0) + "';";
    std::string success_msg =
      "Task " + values.at(0) + " successfully updated!\n";
